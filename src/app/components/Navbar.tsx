  ...
  const handleNavigation = (option: NavOption) => {
    if (option.href) {
      ...
      if (pathname === targetPath) {
        const targetElement = option.hash.slice(2);
        const el = document.getElementById(targetElement);
        el?.scrollIntoView({ behavior: "smooth" });
      } else {
        router.push(option.hash);
      }
    }
              ...
              {navItems.map((item) =>
                item.children ? (
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-full left-0 bg-white shadow-lg rounded-lg z-50 transition-all duration-200 ${
